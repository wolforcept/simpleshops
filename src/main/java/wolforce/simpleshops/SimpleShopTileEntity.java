...
//	}
    ...
    public ItemStack insertInv(Player player, ItemStack stackToInsert) {
        ...
        if (!stackToInsert.is(output.getItem()) || !tagsEqualOrNull(stackToInsert, output) || stackToInsert.getCount() < countToInsert)
    ...
    public void tryBuy(Player player, ItemStack input, boolean isCreative) {
        if (getStockNr() > 0 || isCreative) {
            ...
            if (input.is(cost.getItem()) && input.getCount() >= cost.getCount()) {
